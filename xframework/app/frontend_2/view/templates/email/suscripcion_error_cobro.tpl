        ...
        <tr>
            <td bgcolor="#F7F7F7" style="padding: 24px 0">
                ...
                <table width="600" align="center" bgcolor="#ffffff" border="0" valign="top" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
                    <tr>
                        <td style="padding:16px 16px 0px 16px">
                            ...
                            <p style="color:#415B70; font-size:16px; font-family:'Helvetica', Arial, sans-serif">
                                {if $usuario.idioma_predeterminado =="fr"} Il n'a pas été possible de débiter votre abonnement pour le montant de &euro;{math equation="total/100" total=$monto assign=total}{$total|number_format:2}. 
                                {else}
                                    It was not possible to debit your subscription for the amount of &euro;{math equation="total/100" total=$monto assign=total}{$total|number_format:2}.
                                {/if}
                            </p>
                            {if $descargar_pdf!="" && $pagar_link!=""}
                                <p style="color:#415B70; font-size:16px; font-family:'Helvetica', Arial, sans-serif">
                                    {if $usuario.idioma_predeterminado =="fr"}  Nous fournissons les liens pour télécharger votre facture ou payer la période correspondante:
                                    {else}
                                        We provide links to download your invoice or pay the corresponding period:
                                    {/if}
                                </p>

                                <p style="text-align: center; color:#415B70; font-size:16px; font-family:'Helvetica', Arial, sans-serif">
                                    <a href="{$descargar_pdf}" target="_blank" style="margin-right: 50px; padding: 8px 16px; color: #fff; background-color: #7e746f; text-decoration: none; border-radius: 4px;">{if $usuario.idioma_predeterminado =="fr"}Facture{else}Invoice {/if}</a>
                                    <a href="{$pagar_link}" target="_blank" style="padding: 8px 16px; color: #fff; background-color: #31a592; text-decoration: none; border-radius: 4px;">{if $usuario.idioma_predeterminado =="fr"}Payer{else}Pay {/if}</a>
                                </p>
                            {/if}
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:32px 16px 0px 16px">

                            <p style="color:#415B70; font-size:16px; font-family:'Helvetica', Arial, sans-serif">
                                {if $usuario.idioma_predeterminado =="fr"}  Veuillez vous connecter à votre compte et mettre à jour les informations de votre compte de facturation:
                                {else}
                                    Please login to your account and update your billing account information:
                                {/if}
                            </p>
                            <p style="text-align: center; color:#415B70; font-size:16px; font-family:'Helvetica', Arial, sans-serif">
                                <a href="{$url}entreprises/factures.html" target="_blank" style="padding: 8px 16px; color: #fff; background-color: #F95450; text-decoration: none; border-radius: 4px;">         {if $usuario.idioma_predeterminado =="fr"}Se connecter{else} Log in{/if}</a>
                            </p>
                        </td>
                    </tr>
