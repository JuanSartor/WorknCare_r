...
{literal}
		...
		onFileSuccess: function(file,response) {
	        ...
	        if (parseInt(status)>0 ){
    			...
    			$('img_thumb').setProperty("src","common.php?action=1&modulo=images&submodulo=getThumb&w=50&h=50&image="+ myJSON.image +"&ms="+new Date().getTime() );//random time parameter
...
function removeImg(){
    ...
    var myRequest = new Request({
         ...
         onSuccess: function(responseText, responseXML){
            ...
            if (parseInt(status)>0 ){
                alert(myJSON.msg);
                $('img_temp').setProperty("src","common.php?action=1&modulo=images&submodulo=getThumb&w=50&h=50&image=temp/images/no_image.png" +"&ms="+new Date().getTime() );               
            }else{
                 alert(myJSON.error);
            }                                                              
