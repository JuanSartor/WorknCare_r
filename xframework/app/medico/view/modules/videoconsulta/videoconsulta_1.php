...
if ($this->request["id"] != "") {
    $idvideoconsulta = $this->request["id"];
} else {
...
if ($idvideoconsulta) {
    ...
    if (strtotime($videoconsulta["inicio_sala"]) > strtotime(date("Y-m-d H:i:s"))) {
        ...
        if ((int) $segundos_restantes < (60 * 60)) {//si es en menos de 1 hs
            ...
            $tiempo_proxima = "$min_restantes min";
            $this->assign("tiempo_proxima", $tiempo_proxima);
        } else {
            //la proxima videoconsulta es en mas de 1hs
            $tiempo_proxima = fechaToString($videoconsulta["inicio_sala"], 1) . "hs.";
            $this->assign("tiempo_proxima", $tiempo_proxima);
        }
    } else {
...
// <-- LOG
$log["data"] = "Patient personal info and health profile";
$log["page"] = "Video Consultation";
$log["action"] = "vis"; //"val" "vis" "del"
$log["purpose"] = "See Patient data VIRTUAL PRACTICE";
