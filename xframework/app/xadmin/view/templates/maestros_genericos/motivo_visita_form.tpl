...
<div id="colRight">
    ...
    <div class="block">
        ...
        <div class="title_bar"><div class="text">MOTIVO VISITA &raquo; {if $record} EDITAR {else} NUEVA{/if}</div>   
        ...
        <div class="contenido">
            ...
            <div class="xForm ">
                <form action="{$controller}.php?action=1&modulo=maestros_genericos&submodulo=motivo_visita_form"  method="post"  name="f_record" id="f_record" >
                    <input type="hidden" name="idmotivoVisita" id="idmotivoVisita" value="{$record.idmotivoVisita}" />

                    <div class="form_container ">
                        <div class="title">Motivo Visita</div>
                        <ul>

                            <li class="wide">
                                <label>Motivo * </label> 
                                {x_form_input  id="motivoVisita" descriptor="none" isRequired="true" maxChars=100 class="" record=$record} 
                            </li>  
                        ...
                        <div id="listado_especialidades" {if $listado_especialidades}style="display:block;"{else}style="display:none"{/if}>
                            ...
                            <ul>
                                {foreach from=$listado_especialidades item=especialidad}
                                    <li data-id="{$especialidad.idmotivovisita_especialidad}" data-especialidad="{$especialidad.especialidad_idespecialidad}" class="especialidad"><span class="delete_especialidad" data-id="{$especialidad.idmotivovisita_especialidad}" title="Eliminar"> X </span> {$especialidad.especialidad} </li>
                                    {/foreach}
                            </ul>
                        </div>
                        ...
                        <div id="listado_programas_categorias" {if $listado_programas_categorias}style="display:block;"{else}style="display:none"{/if}>
                            ...
                            <ul>
                                {foreach from=$listado_programas_categorias item=programa_categoria}
                                    <li data-id="{$programa_categoria.idmotivovisita_programa_categoria}" data-programa_categoria="{$programa_categoria.idprograma_categoria}" class="programa_categoria"><span class="delete_programa_categoria" data-id="{$programa_categoria.idmotivovisita_programa_categoria}" title="Eliminar"> X </span> {$programa_categoria.programa_salud} - {$programa_categoria.programa_categoria} </li>
                                    {/foreach}
                            </ul>
                        </div>
                        <div class="clear">&nbsp;</div>
