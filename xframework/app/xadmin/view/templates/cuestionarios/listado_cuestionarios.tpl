...
<div class="title" style="cursor:pointer;margin-bottom: 16px;">Cuestionario Asignados</div>
<!--<li class="wide">
...
<div id="listado_cuestionarios" {if $listado_cuestionarios}style="display:block;" {else}style="display:none" {/if}>
    <ul>
        {foreach from=$listado_cuestionarios item=cuestionario}
            <li data-id="{$cuestionario.idcuestionario}"  class="programa"><span class="delete_cuestionario" data-id="{$cuestionario.idcuestionario}" title="Eliminar"> X </span> {$cuestionario.titulo}</li>
            {/foreach}
    </ul>
