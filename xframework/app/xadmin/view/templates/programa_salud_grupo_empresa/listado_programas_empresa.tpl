...
<div id="listado_programas" {if $listado_programas}style="display:block;" {else}style="display:none" {/if}>
    <ul>
        {foreach from=$listado_programas item=programa}
            <li data-id="{$programa.idprogramas_salud_grupo_empresa_asociacion}"  class="programa"><span class="delete_programa" data-id="{$programa.idprogramas_salud_grupo_empresa_asociacion}" title="Eliminar"> X </span> {$programa.programa_salud}</li>
            {/foreach}
    </ul>
