...
$("#submodulo").val("notificaciones_medico_form");
    ...
    if (ids.length > 0 || $("#idnotificacionSistema").val() != "") {
        ...
        x_sendForm(
                ...
                function (data) {
                    x_alert(data.msg);
                    if (data.result) {
                        x_loadModule('notificaciones', 'notificaciones_medico_list', '', 'Main')
                    }

                }
        );
    } else {
        x_alert("No ha seleccionado medicos para enviar la notificación");
    }
});

$("#back").click(function () {
    x_goTo('notificaciones', 'notificaciones_medico_list', '', 'Main', this);
});
