...
//verificamos si el paciente puede sacar consultas a medicos de LUX
        ...
        $motivos = $this->getManager("ManagerMotivoVisita")->getComboByEspecialidad($medico["mis_especialidades"][0]["idespecialidad"]);
        $this->assign("combo_motivo_visita", $motivos);
        ...
        if ((int) $this->request["idprograma_categoria"] > 0) {
            ...
            $motivos = $this->getManager("ManagerMotivoVisita")->getComboByProgramaCategoria($this->request["idprograma_categoria"]);
            $this->assign("combo_motivo_visita", $motivos);
        } else {
        ...
        $obra_social = $this->getManager("ManagerObraSocialPaciente")->getObraSocialPaciente($paciente["idpaciente"]);

        if ($obra_social) {
            $this->assign("obra_social_paciente", $obra_social);
            $this->assign("combo_planes_obra_social", $this->getManager("ManagerPlanesObrasSociales")->getCombo($obra_social["obraSocial_idobraSocial"]));
        }
    } else {
