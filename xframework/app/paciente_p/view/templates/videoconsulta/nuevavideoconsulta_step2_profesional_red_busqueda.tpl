...
<section class="container cs-nc-p2 cs-nc-profesionales-en-la-red">
    ...
    <div class="cs-nc-p2-inner">
        <div class="container-fluid">
            <form name="f_busqueda" id="f_busqueda" action="{$url}busqueda_profesionales_red_.do" method="POST" role="form" onsubmit="return false">
                ...
                {if $login_prestador!=1}
                    <input type="hidden" id="rango_maximo" name="rango_maximo" value="{$PRECIO_MAXIMO_VC}">
                    <input type="hidden" id="rango_minimo" name="rango_minimo" value="{$PRECIO_MINIMO_VC}">
            ...
            <div class="cs-range-mas-filtros-trigger-holder">
                ...
                <div class="cs-nc-listado-p2-siguiente pull-right">
                    <a id="btnContinuar" class="cs-nc-p2-btn-siguiente" href="javascript:void(0)" style="margin-top:0px">{"continuar"|x_translate} <span></span></a>
    ...
    {literal}
        <script>
            $(function () {
                ...
                x_loadModule('videoconsulta', 'rango_precios_profesionales_red', '', 'container_rango_precios', BASE_PATH + "paciente_p");


                $("#especialidad_idespecialidad").change(function () {
                    $("#idpais_f").val($("#idpais").val());
                    x_loadModule('videoconsulta', 'rango_precios_profesionales_red', $("#f_busqueda").serialize(), 'container_rango_precios', BASE_PATH + "paciente_p");
...
{literal}
    <script>
        ...
        $(function () {
            ...
            x_doAjaxCall(
                    'POST',
                    BASE_PATH + 'delete_filtros_busqueda_vc.do',
            ...
            $('#as_localidad').autocomplete({
                zIndex: 10000,
                serviceUrl: BASE_PATH + 'paciente_p.php?action=1&modulo=videoconsulta&submodulo=localidad_autosuggest&pais_idpais=' + $("#idpais").val(),
            ...
            //actualizamos el autosuggest seleccionando una provincia
            $("#idpais").change(function () {

                $("#idlocalidad, #as_localidad").val("");
                $('#as_localidad').autocomplete('clear');
                $('#as_localidad').autocomplete('setOptions', {serviceUrl: BASE_PATH + 'paciente_p.php?action=1&modulo=videoconsulta&submodulo=localidad_autosuggest&pais_idpais=' + this.value});
            });
            ...
            $("#btnContinuar").click(function () {
                ...
                x_doAjaxCall(
                        'POST',
                        BASE_PATH + "save_filtros_busqueda_vc.do",
                        $("#f_busqueda").serialize(),
                        function (data) {
                            if (data.result) {
                                if (data.login_prestador == 1) {
                                    x_loadModule('videoconsulta', 'nuevavideoconsulta_step3', 'idvideoconsulta=' + $("#idvideoconsulta_f").val(), 'videoconsulta-step-container', BASE_PATH + "paciente_p");

                                } else {
                                    x_loadModule('videoconsulta', 'nuevavideoconsulta_step2_profesional_red_resultados', "idvideoconsulta=" + $("#idvideoconsulta_f").val() + "&do_reset=1", "videoconsulta-step-container", BASE_PATH + "paciente_p");
                                }
                            } else {
