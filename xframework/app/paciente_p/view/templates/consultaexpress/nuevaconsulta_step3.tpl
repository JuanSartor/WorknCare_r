<section class="container ce-nc-consulta ce-nc-p4-usr">
    ...
    <div class="row" id="div_alerta_confirmacion" style="padding-top:30px" >
        {include file="consultaexpress/banner_alerta_confirmacion_urgencia.tpl"}
    </div>
    <div class="clearfix">
        &nbsp;
    </div>
    <form id="f_publicar" name="f_publicar" action="{$url}publicar_consulta.do" method="post"  onsubmit="return false;" style="display:none;">
        <input type="hidden" name="cantidad" id="cantidad" value=""/>
        <input id="idconsultaExpress_f" name="idconsultaExpress" value="{$ConsultaExpress.idconsultaExpress}" type="hidden" >
        ...
        <div class="row">
            ...
            <div class="{if $combo_programas}col-sm-5{else}col-sm-10 col-sm-offset-1{/if} ce-nc-p4-mensaje">
                ...
                <div class="cs-nc-p2-input-holder">
                    <select id="motivoConsultaExpress" name="motivoConsultaExpress_idmotivoConsultaExpress"   {if $combo_programas}disabled="disabled"{/if} class="form-control select select-primary select-block mbl">
                        <option value="">{"Seleccione una opción"|x_translate}</option>
                        {html_options options=$combo_motivos selected=$ConsultaExpress.motivoConsultaExpress_idmotivoConsultaExpress}
                    </select>
                </div>
            </div>
        </div>
...
<div class="modal fade bs-example-modal-lg modal-perfil-completo-acreditacion" id="modal-cambio-privacidad-perfil-salud" tabindex="-1" role="dialog"  data-keyboard="false" data-backdrop="static" aria-labelledby="myLargeModalLabel">
...
{if $consulta_pendiente_exist.idconsultaExpress!=""}
            ...
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <h3 class="modal-sub-title">{"Ya has enviado una consulta al profesional"|x_translate}</h4>
                        <h4 class="modal-title">{"Tienes una consulta express pendiente con este profesional. Puedes esperar su respuesta o solicitarle otra consulta nuevamente"|x_translate}</h3>
                </div>
    ...
    <script>
        ...
        $(function () {
            ...
            $(".select-categoria").click(function (e) {
                ...
                updateComboBoxSelect2("#idprograma_categoria", 'motivoConsultaExpress', 'ManagerMotivoConsultaExpress', 'getComboByProgramaCategoria', 0, x_translate('Seleccione una opción'), doUpdateComboBoxSelect2);
            ...
            $("#btnPasoAnterior").click(function () {
                ...
                x_doAjaxCall(
                        'POST',
                        BASE_PATH + 'back_paso_2.do',
                        "idconsultaExpress=" + $("#idconsultaExpress").val() + "&consulta_step=1",
                        function (data) {
            ...
            $("#btnEnviarConsulta").click(function () {
                ...
                if ($("#motivoConsultaExpress").val() === "") {
                    x_alert(x_translate("Seleccione el motivo de su consulta"));
                    $("#motivoConsultaExpress").addClass("select-required");
                    return false;
                }
                ...
                x_sendForm($('#f_publicar'), true, function (data) {
                    ...
                    if (data.result) {
                        x_loadModule('consultaexpress', 'nuevaconsulta_step4', 'idconsultaExpress=' + $("#idconsultaExpress").val(), 'consulta-express-step-container', BASE_PATH + "paciente_p");
                    } else {
                        x_alert(data.msg);
                    }
                });
