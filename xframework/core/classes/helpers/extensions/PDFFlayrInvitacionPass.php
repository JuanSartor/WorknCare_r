...
class PDFFlayrInvitacionPass extends JeyReport {
    ...
    public function getPDF($data, $file = NULL) {
        ...
        if (is_null($file)) {
            $this->Output("flyer-invitation-pass-bienetre.pdf", "I");
        } else {
            @$this->Output($file, "F");
        }
        ob_end_flush();
        ...
        if (CONTROLLER == 'frontend_2' || $empresa["image"] == '') {
            switch ($this->template) {
                case "caratula":
                    ...
                    $this->setSourceFile(path_view('templates/pdf/flyer_workncare.pdf'));
...
//Dibujamos la pagina con QR
    ...
    public function drawInvitacion() {
        ...
        if ($this->data["empresa"]["image"]) {
            $this->Image(path_files($this->data["img"]), 16, 5, 25, 25);
        }

        $this->SetFont('Arial', 'B', 22);
        $this->SetTextColor(118, 118, 118);

        $this->SetXY(10, 49);
        $this->MultiCell(190, 5, utf8_decode($empresa["titulo_flyer"]), 0, 'C', 0);

        $this->SetFont('Arial', '', 18);
        $this->SetTextColor(118, 118, 118);
        $this->SetXY(18, 65);
        $this->MultiCell(190, 5, utf8_decode($empresa["subtitulo_flyer"]), 0, 'L', 0);

        $this->Image(path_files("temp/qr_invitacion/$idempresa.png"), $x - 8, $y - 73, 45, 45);
    }
