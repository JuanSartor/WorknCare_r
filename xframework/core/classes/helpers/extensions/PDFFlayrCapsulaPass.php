...
class PDFFlayrCapsulaPass extends JeyReport {
    ...
    public function getPDF($data, $file = NULL) {
        ...
        if (is_null($file)) {
            $this->Output("flyer-capsule-pass-bienetre.pdf", "I");
        } else {
            @$this->Output($file, "F");
        }
        ob_end_flush();
        ...
        if (CONTROLLER == 'frontend_2' || $empresa["image"] == '') {
            switch ($this->template) {
                case "caratula":
                    ...
                    $this->setSourceFile(path_view('templates/pdf/template_flyers_capsule.pdf'));
...
// use the imported page and place it at point 10,10 with a width of 100 mm 
    ...
    public function drawInvitacion() {
        ...
        $this->MultiCell(170, 5, utf8_decode($this->data["capsula"]["titulo_flyer"]), 0, 'C', 0);

        $this->setXY(19, 113);
        $this->SetFont('Arial', '', 16);
        $this->MultiCell(175, 5, utf8_decode($this->data["capsula"]["subtitulo_flyer"]), 0, 'L', 0);


        if ($this->data["usuario"]["idioma_predeterminado"] == 'fr') {
            $this->setXY(70, 145);
            $this->SetFont('Arial', 'B', 20);
            $this->MultiCell(175, 5, utf8_decode($this->data["24"]["texto_fr"]), 0, 'L', 0);
            $this->setXY(88, 155);
            $this->SetFont('Arial', '', 16);
            $this->MultiCell(175, 5, utf8_decode($this->data["25"]["texto_fr"]), 0, 'L', 0);
        } else {
            $this->setXY(70, 145);
            $this->SetFont('Arial', 'B', 20);
            $this->MultiCell(175, 5, utf8_decode($this->data["24"]["texto_en"]), 0, 'L', 0);
            $this->setXY(88, 155);
            $this->SetFont('Arial', '', 16);
            $this->MultiCell(175, 5, utf8_decode($this->data["25"]["texto_en"]), 0, 'L', 0);
        }


        $this->setXY(19, 255);
        $this->SetFont('Arial', '', 16);
        $this->MultiCell(175, 5, utf8_decode($this->data["capsula"]["complementario_flyer"]), 0, 'L', 0);
        ...
        $this->Image(path_files("temp/qr_invitacion_capsula/$hashc.png"), $x + 4.8, $y - 17, 70, 70);
